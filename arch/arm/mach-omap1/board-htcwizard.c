...
/*
  ...
  UART2 IR?
*/
static struct omap_uart_config htcwizard_uart_config __initdata = {
...
static struct omap_board_config_kernel htcwizard_config[] = 
{
	...
	{ OMAP_TAG_UART, &htcwizard_uart_config },
};
...
struct platform_device htcartemis_bt = {
        .name = "htcartemis-bt",
        .id = 1
};
...
static void __init htcwizard_init(void)
{
  ...
  // Add UART2 clock activation
  omap_writel(omap_readl(0xfffe0834) | 0x800, 0xfffe0834);
  // Add UART1 clock activation
...
MACHINE_START(OMAP_HTCWIZARD, "HTC Prophet")
