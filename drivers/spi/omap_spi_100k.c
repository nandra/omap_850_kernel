...
/*
        ...
        status = request_irq(irq, spi100k_int, SA_SAMPLE_RANDOM | SA_TRIGGER_LOW | SA_TRIGGER_HIGH, pdev->dev.bus_id, master);
        if (status < 0) {
...
*/
        ...
        spi100k->base_addr = (unsigned long)r->start; //ioremap((unsigned long)r->start, (r->end - r->start) + 1);
